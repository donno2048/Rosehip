...
class SnakeCalc(pygame_gui.elements.UIWindow):
    ...
    def __init__(self, pos, manager):
        ...
        for i in range(len(self.OPS)):
            op = self.OPS[i]
            if op != "x":pygame_gui.elements.UIButton(relative_rect=pygame.Rect((i % 4 * self.BSIZE[0], 40 + int(i / 4) * self.BSIZE[1]), self.BSIZE),text="." if op == "p" else op,manager=manager,container=self,object_id="#op-" + op,)
...
def load(manager, params):pos = params[0] if params is not None and len(params) > 0 else (100,100);SnakeCalc(pos, manager)
